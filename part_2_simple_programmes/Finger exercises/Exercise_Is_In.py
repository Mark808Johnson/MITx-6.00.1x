...
def isIn(char, aStr):
    ...
    if char == aStr or char == middle:
        return True
    elif len(aStr) <= 1 and char != aStr:
        return False
    elif alp.find(char) < alp.find(middle):
...
print(f"Is '{letter1}' in alphabetically-ordered string '{string}'?")
print(f"Expected answer: False")
print(f"Computed answer: {isIn(letter1, string)}")
print("_______")
print(f"Is '{letter2}' in string '{string}'?")
print(f"Expected answer: True")
print(f"Computed answer: {isIn(letter2, string)}")
